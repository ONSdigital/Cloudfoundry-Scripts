...
DEPLOYMENT_NAME="$1"
BOSH_MANIFEST_NAME="${2:-Bosh-Template}"
...
#
BOSH_LITE_STATE_FILE="$DEPLOYMENT_FOLDER/$BOSH_MANIFEST_NAME-Lite-state.json"
BOSH_LITE_VARS_FILE="$DEPLOYMENT_FOLDER/$BOSH_MANIFEST_NAME-Lite-vars.yml"
BOSH_FULL_VARS_FILE="$DEPLOYMENT_FOLDER/$BOSH_MANIFEST_NAME-Full-vars.yml"
#
BOSH_LITE_MANIFEST_FILE="$MANIFESTS_DIR/Bosh-Lite-Manifests/$BOSH_MANIFEST_NAME.yml"
BOSH_FULL_MANIFEST_FILE="$MANIFESTS_DIR/Bosh-Full-Manifests/$BOSH_MANIFEST_NAME.yml"
BOSH_FULL_CLOUD_CONFIG_FILE="$MANIFESTS_DIR/Bosh-Full-Manifests/$BOSH_MANIFEST_NAME-AWS-CloudConfig.yml"
#
SSL_FOLDER="$DEPLOYMENT_FOLDER/ssl"
SSL_FOLDER_RELATIVE="$DEPLOYMENT_FOLDER_RELATIVE/ssl"
SSL_YML="$SSL_FOLDER/ssl_config.yml"
#
AWS_OUTPUTS_CONFIG_FILE="$DEPLOYMENT_FOLDER/outputs.sh"
...
[ -z "$BOSH_MANIFEST_NAME" ] && FATAL 'No Bosh manifest name provided'
[ -d "$MANIFESTS_DIR" ] || FATAL "$MANIFESTS_DIR directory does not exist"
[ -f "$AWS_OUTPUTS_CONFIG_FILE" ] || FATAL "AWS Cloudformation outputs file '$AWS_OUTPUTS_CONFIG_FILE' does not exist"
...
eval export `prefix_vars "$AWS_OUTPUTS_CONFIG_FILE" "$ENV_PREFIX"`
