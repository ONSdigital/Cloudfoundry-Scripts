...
[ -n "$PRIVATE_BOSH_LITE_OPS_FILE_NAME" -a -f "$CONFIGS_DIR/ops-files/$PRIVATE_BOSH_LITE_OPS_FILE_NAME.yml" ] && PRIVATE_BOSH_LITE_OPS_FILE="$CONFIGS_DIR/ops-files/$PRIVATE_BOSH_LITE_OPS_FILE_NAME.yml"
[ -n "$PRIVATE_BOSH_FULL_OPS_FILE_NAME" -a -f "$CONFIGS_DIR/ops-files/$PRIVATE_BOSH_FULL_OPS_FILE_NAME.yml" ] && PRIVATE_BOSH_FULL_OPS_FILE="$CONFIGS_DIR/ops-files/$PRIVATE_BOSH_FULL_OPS_FILE_NAME.yml"

# Publically available ops files, eg adjustments for VMware
if [ -n "$PUBLIC_BOSH_LITE_OPS_FILE_NAME" -a -f "$MANIFESTS_DIR/Bosh-Lite-Manifests/$PRIVATE_BOSH_LITE_OPS_FILE_NAME.yml" ]; then
	PUBLIC_BOSH_LITE_OPS_FILE="$MANIFESTS_DIR/Bosh-Lite-Manifests/$PRIVATE_BOSH_LITE_OPS_FILE_NAME.yml"
fi

if [ -n "$PUBLIC_BOSH_FULL_OPS_FILE_NAME" -a -f "$MANIFESTS_DIR/Bosh-Lite-Manifests/$PRIVATE_BOSH_FULL_OPS_FILE_NAME.yml" ]; then
	 PUBLIC_BOSH_FULL_OPS_FILE="$MANIFESTS_DIR/Bosh-Lite-Manifests/$PRIVATE_BOSH_FULL_OPS_FILE_NAME.yml"
fi
